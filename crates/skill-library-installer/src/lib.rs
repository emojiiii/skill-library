...
use skill_library_core::{RuntimeTarget, SkillLibraryError};
use skill_library_manifest::{parse_skill_dir, SkillManifest};
use std::fs;
...
pub fn install(options: InstallOptions) -> Result<InstallReport> {
    ...
    for target in requested {
        ...
        let temp_parent = root.join(".skill-library-tmp");
        ...
        let metadata = InstallMetadata {
            ...
            target: target.clone(),
            managed_by: "skill-library".to_owned(),
        };
        fs::write(
            staging.path().join(".skill-library-install.json"),
            serde_json::to_vec_pretty(&metadata)
                .map_err(|err| InstallerError::InvalidSource(err.to_string()))?,
        )?;
        if final_dir.exists() {
            let backup = root.join(format!(".{}.skill-library-backup", manifest.id));
...
pub fn list_installed(target: &str, roots: Vec<TargetRoot>) -> Result<Vec<InstallMetadata>> {
    ...
    for entry in fs::read_dir(root)? {
        ...
        let metadata = entry.path().join(".skill-library-install.json");
        if metadata.exists() {
...
fn rename_staging_into_place(staging_dir: &Path, final_dir: &Path) -> std::io::Result<()> {
    if std::env::var_os("SKILL_LIBRARY_INSTALLER_INJECT_SWAP_FAILURE").is_some()
        && staging_dir.join(".skill-library-fail-swap").exists()
    {
...
fn should_exclude(path: &Path) -> bool {
    path.components().any(|component| {
        ...
        matches!(
            value.as_ref(),
            ".git" | "node_modules" | ".DS_Store" | ".skill-library-install.json"
        )
    })
}
...
"#,
        ...
        assert!(target
            .path()
            .join("code-reviewer/.skill-library-install.json")
            .exists());
    }
