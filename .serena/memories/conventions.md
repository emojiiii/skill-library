...
- App state lives under `~/.skill-library` via `AppPaths`; tests/smokes should use isolated HOME or explicit target roots to avoid touching real Claude Code/Cursor/Codex state.
...
- Desktop has API-unavailable demo fallbacks in `apps/desktop/src/lib/skill-library.ts`; keep real-provider paths separate from fallback data.
