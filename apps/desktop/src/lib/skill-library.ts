...
const desktopOnly = (op: string): never => {
  throw new Error(
    `${op} requires the Skill Library desktop app (Tauri). Run \`pnpm dev\` (which calls \`tauri dev\`) instead of opening the Vite dev URL in a browser.`,
  );
};
...
export async function onDeepLink(
  ...
  return listen<DeepLinkPayload>("skill-library://deep-link", (event) => handler(event.payload));
}
...
export async function logoutGithub(): Promise<void> {
  if (!isTauri) return desktopOnly("Sign out of GitHub");
...
export async function dbAddProjectDeployments(
  skillId: string,
  projectTargets: ProjectInstallTarget[],
): Promise<void> {
  if (!isTauri) return desktopOnly("Add project deployment");
  return invoke("db_add_project_deployments", { skillId, projectTargets });
}
