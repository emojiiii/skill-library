...
pub type Result<T> = std::result::Result<T, TeamAIError>;
...
impl AppPaths {
    pub fn resolve() -> Result<Self> {
        let home_dir = dirs::home_dir()
            .ok_or_else(|| TeamAIError::user("cannot resolve the current user's home directory"))?;
        Ok(Self::for_home(home_dir.join(".team-ai-hub")))
    }

    pub fn for_home(home: PathBuf) -> Self {
        Self {
            config: home.join("config.toml"),
            database: home.join("teamai.db"),
    ...
    pub fn ensure(&self) -> Result<()> {
        ...
        if !self.config.exists() {
            let config = toml::to_string_pretty(&TeamAIConfig::default())?;
...
fn github_credential_entry() -> std::result::Result<keyring::Entry, TeamAIError> {
