...
impl GitHubProvider {
    ...
    pub fn with_base_url(api_base: impl Into<String>, token: Option<String>) -> Result<Self> {
        ...
        headers.insert(USER_AGENT, HeaderValue::from_static("team-ai-hub/0.1"));
        headers.insert(
            ACCEPT,
    ...
    async fn get_json<T: for<'de> Deserialize<'de>>(&self, path: &str) -> Result<T> {
        ...
        tracing::debug!(target: "teamai-github", method = "GET", path);
        let response =
            self.client
                .get(url)
                .send()
    ...
    async fn post_json<B: Serialize, T: for<'de> Deserialize<'de>>(
        ...
        let url = format!("{}{}", self.api_base, path);
        tracing::debug!(target: "teamai-github", method = "POST", path);
        let response = self
            .client
            .post(url)
            .json(body)
    ...
    async fn put_json<B: Serialize, T: for<'de> Deserialize<'de>>(
        ...
        let url = format!("{}{}", self.api_base, path);
        tracing::debug!(target: "teamai-github", method = "PUT", path);
        let response = self
            .client
            .put(url)
            .json(body)
    ...
    pub async fn graphql<T: for<'de> Deserialize<'de>>(
        ...
        tracing::debug!(target: "teamai-github", method = "POST", path = "/graphql");
        let response = self
            .client
            .post(url)
        ...
        if !envelope.errors.is_empty() {
            let summary = envelope
                ...
                .collect::<Vec<_>>()
                .join("; ");
            tracing::warn!(target: "teamai-github", errors = %summary, "graphql errors");
