# Skill Library Real-Provider Demo Runbook

Use this runbook to execute and record the final MVP demo against a real GitHub repository named `skill-library-demo-skills`.
...
- GitHub repository: `OWNER/skill-library-demo-skills`.
...
export SKILL_LIBRARY_WEBHOOK_CALLBACK_URL=https://YOUR-TUNNEL.example/api/webhooks/github
rtk pnpm dev:api
```
...
By default this creates `./skill-library-demo-skills`. Push that repository to GitHub as `OWNER/skill-library-demo-skills` before running the real-provider script.
...
export GITHUB_TOKEN=...
export SKILL_LIBRARY_API=http://localhost:8787
```
...
1. Login:
   ...
   ```bash
   rtk cargo run -p skill-library-cli -- login github --token "$GITHUB_TOKEN"
   rtk cargo run -p skill-library-cli -- auth status
   ```

2. Add and scan the workspace:

   ```bash
   rtk cargo run -p skill-library-cli -- workspace add "$SKILL_LIBRARY_DEMO_WORKSPACE" \
     --token "$GITHUB_TOKEN" \
     --webhook-url "$SKILL_LIBRARY_WEBHOOK_CALLBACK_URL" \
     --webhook-secret "$GITHUB_WEBHOOK_SECRET" \
     --webhook-event push \
     --webhook-event release
   rtk cargo run -p skill-library-cli -- scan-remote "$SKILL_LIBRARY_DEMO_WORKSPACE" --token "$GITHUB_TOKEN"
   ```
...
4. Subscribe and sync:
   ...
   ```bash
   rtk cargo run -p skill-library-cli -- subscribe "$SKILL_LIBRARY_DEMO_WORKSPACE" code-reviewer \
     --target claude-code \
     --target cursor \
     --target codex \
     --update auto-patch
   rtk cargo run -p skill-library-cli -- sync \
     --token "$GITHUB_TOKEN" \
     --pull-notifications \
     --api "$SKILL_LIBRARY_API" \
     --yes
   rtk cargo run -p skill-library-cli -- status \
     --target claude-code \
     --target cursor \
     --target codex
   ```
...
5. Publish local Skill:
   ...
   ```bash
   rtk cargo run -p skill-library-cli -- package ~/.claude/skills/local-helper \
...
6. Invite a member:
   ...
   ```bash
   rtk cargo run -p skill-library-cli -- invite "$SKILL_LIBRARY_DEMO_WORKSPACE" GITHUB_USERNAME \
     --role read \
     --token "$GITHUB_TOKEN" \
     --api "$SKILL_LIBRARY_API"
   ```
...
7. Update:
   ...
   rtk cargo run -p skill-library-cli -- notifications --api "$SKILL_LIBRARY_API" --repository "$SKILL_LIBRARY_DEMO_WORKSPACE"
   rtk cargo run -p skill-library-cli -- sync \
     --token "$GITHUB_TOKEN" \
     --pull-notifications \
     --api "$SKILL_LIBRARY_API" \
...
8. Rollback:
   ...
   ```bash
   rtk cargo run -p skill-library-cli -- rollback "$SKILL_LIBRARY_DEMO_WORKSPACE" code-reviewer v1.2.0 \
     --target claude-code \
     --target cursor \
     --target codex \
     --token "$GITHUB_TOKEN" \
...
- `.skill-library-demo-evidence/<timestamp>/00-cargo-test.log`, `01-pnpm-check.log`, and `02-pnpm-test.log` showing all checks passed.
...
- `09-status-after-sync.log` from `skill-library status --target claude-code --target cursor --target codex` after sync.
