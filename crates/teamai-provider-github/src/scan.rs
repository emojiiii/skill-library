...
pub async fn read_skill_detail(
    ...
    // if nothing is there, scan the repo once and resolve by manifest id or the
...
/// the scan already parsed. Matches by manifest id or the directory's basename.
...
/// manifest id, full path, or final path component equals `needle`. Match
/// priority: exact path > manifest id > basename, so a literal path always
...
mod tests {
    ...
    /// Matching by manifest id works even when it differs from the directory name.
    #[test]
