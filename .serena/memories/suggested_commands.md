...
- Rust targeted tests: `rtk cargo test -p teamai-cli`, `rtk cargo test -p teamai-sync`.
...
- Verify final evidence: `rtk pnpm demo:verify-evidence .teamai-demo-evidence/<timestamp>`.
- Docker config sanity: `rtk docker compose config`.
- Dev servers: `rtk pnpm dev:desktop`, `rtk pnpm dev:api`.
- Run CLI from source: `rtk cargo run -p teamai-cli -- --help`.