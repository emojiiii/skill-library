# Skill Library 产品文档 v2.1

## 1. 产品概述

Skill Library 是一个 **基于 Git Provider 之上的团队 AI Skills 工作流层**。

团队把 Skills 放在 Git 仓库里（GitHub 优先，其他 Provider 后续支持），Skill Library 提供：
...
- **邀请与 onboarding** —— 在 Skill Library 里邀请同事，底层仍然落到 GitHub / GitLab / Gitea 的成员关系和仓库权限。
...
> **Vercel 之于代码仓库的关系，就是 Skill Library 之于 AI 资产仓库的关系。**
...
`skills` CLI 已经覆盖 Claude Code、Cursor、Codex 等 agent 的安装路径、全局/项目级 scope、add/list/update/remove/init 等基础能力，是重要参考实现。但 Skill Library 桌面端不内置 Node / Bun，MVP 用 Rust 实现自己需要的最小 installer，把工程重心放在团队订阅、发布 PR、权限校验、风险策略和可视化治理上。
...
Skill Library 可以提供"邀请同事加入团队"的入口、邀请收件箱、接受邀请后的 onboarding 页面，但最终成员关系必须落到 GitHub / GitLab / Gitea / GitHub Enterprise 等 Provider。我们不维护影子成员表，不让 bot 成为权限绕过通道。
...
- 在 Skill Library 里邀请成员，本质是调用 Provider 的 org/team/repo collaborator 邀请 API。
...
```yaml
# ~/.skill-library/subscriptions.yaml
subscriptions:
...
发布声明 "把我本地的个人 Skill 同步到某个团队 Workspace"。底层不是直接写主分支，而是由 Skill Library Bot 代表用户创建 branch + PR：

```text
本地个人 Skill
→ skill-library publish code-reviewer --workspace acme/team-skills
→ 校验发起人对目标 repo 至少有 write 权限
→ 打包 Skill，计算 hash，生成 manifest / risk summary
→ Bot 创建 branch: skill-library/import/code-reviewer/<short-sha>
...
> Skill Library Bot 只代表用户执行 Git 操作，不提升用户权限。
...
| GitHub Enterprise / GitLab / Gitea 内网 | 自托管 Skill Library 调用对应 Provider 的成员 API |

如果被邀请人还没有 GitHub 账号，Skill Library 可以展示注册引导并把用户带到 GitHub 注册；注册完成后再回到 Skill Library 完成 Provider 邀请接受。我们不直接创建 GitHub 账号。
...
Skill Library Bot 可以拥有 repo 写权限或成员邀请权限，但每次执行前必须用发起人的 Provider 身份做实时权限校验：
...
- **个人订阅**：`~/.skill-library/subscriptions.yaml`
- **团队订阅**：仓库内 `.skilllibrary/subscriptions.yaml`，团队所有成员共享
...
- **手动**：CLI `skill-library sync` 或 Web UI 点击。
...
- **CLI**：`skill-library` 命令行工具，headless 操作，CI/CD 友好。
...
```bash
skill-library login github                        # OAuth 登录
skill-library workspace add acme/team-skills      # 添加订阅源
skill-library workspace list
skill-library search code                         # 搜索可订阅资产
skill-library subscribe code-reviewer             # 订阅
skill-library unsubscribe code-reviewer
skill-library sync                                # 立即同步
skill-library status                              # 查看本地状态
skill-library versions code-reviewer              # 查看版本历史
skill-library rollback code-reviewer 1.4.1        # 回滚
skill-library diff code-reviewer 1.4.1 1.4.2      # 版本对比
skill-library enable code-reviewer --target cursor
skill-library disable code-reviewer --target codex
```

## 11. Rust Native Installer 与 Runtime 安装

MVP 使用 Rust 原生 installer + SQLite 注册表，把 Skill 安装到各 agent 期望的位置。Skill Library 负责订阅、版本、权限提示、policy 和 rollback。

### 11.1 架构

- **Canonical 数据目录**：`~/.skill-library/skills/{id}/` 存放 skill 的唯一真实副本。
- **Symlink 分发**：启用某个 IDE 时，创建 symlink 指向 canonical 目录。
- **SQLite 注册表**：`~/.skill-library/db.sqlite` 记录所有 skill 的元数据、启用状态、变更 hash。
...
Skill Library Bot 的 token 只用于执行已授权操作。任何写入、PR、merge、邀请动作前，都必须先用发起人的 Provider 身份检查权限：

- 发起人没有 read 权限：不可见，不可安装。
- 发起人没有 write 权限：不可 publish 到团队 repo。
- 发起人没有 admin / owner / maintainer 权限：不可通过 Skill Library 发邀请或 auto-merge。
...
Skill Library 的注册就是 Provider 登录：

- 已有 GitHub 账号：点击 "Continue with GitHub" 即可进入。
- 受邀但没有 GitHub 账号：先跳转到 GitHub 注册，再回到 Skill Library 接受邀请。
- 企业内网：使用自托管 Provider 的 OAuth / SSO / PAT，Skill Library 不直接管理企业身份源。
...
```bash
skill-library --no-cloud subscribe github.com/acme/team-skills/code-reviewer
```
...
### Phase 2 — 团队策略与设备管理
团队级 `.skilllibrary/subscriptions.yaml`、设备管理、publish policy、auto-merge policy、更多 agent target 配置。
...
### 18.4 一句话差异化
> 现有方案要么被绑在单一 IDE / Agent 上，要么停在"一个共享 Git repo"的原始阶段。Skill Library 把它们之间的鸿沟填上 —— 借助 Git Provider 已有的协作能力，提供跨 runtime 的订阅、同步、版本管理与可视化体验。
...
- 邀请：Skill Library 提供邀请体验，但最终成员关系必须落到 Git Provider。
...
> Skill Library = **Git Provider 上的团队 Skills 工作流层**：仓库即团队空间，PR 即发布，Provider 即权限源 —— 我们补上订阅、同步、邀请、风险提示、版本治理与非工程师入口。
